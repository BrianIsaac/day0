...
9 月 12 日决赛补充：证据卡还包含经 Featherless 托管的 `zai-org/GLM-5.3-Flash` **仅 baseline** 结果（45 行，charter 准备失败，没有入职组）。内容均为合成评测数据。下述三个环境是半决赛清单，并非决赛的全部清单。预算细节来自运行者记录，未保留原始 charter HTTP 回复。
